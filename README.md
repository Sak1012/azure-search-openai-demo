...
```
3. Switch to `p4ai` branch:
```bash
git fetch origin p4ai
git switch p4ai
```
4. Open the config file for P4AI:

```bash
code .\azure-search-openai-demo\app\frontend\src\p4ai\config.ts
```
5. Replace the `CLIENT_ID` and `TENANT_ID` with the id from App registration and your tenant.
```ts
export const config = {
...
```
6. Continue with Deployment and Development Server.
